...
class TesiraPlugin(ManualPlatformPlugin):
    ...
    def _has_terminator(self, output):
        return any(
            line.strip().startswith(t)
            for line in output.splitlines()
            for t in _TERMINATORS
        )
 
    def _drain_shell(self, shell, idle_timeout=_PRE_COMMAND_IDLE, max_timeout=1.0):
        """
        Drain delayed bytes from the previous command before sending a new one.
        Tesira can echo/respond a little late; without this, replies shift by one
        command and device_info fields become scrambled.
        """
        drained = ""
        idle_deadline = time.monotonic() + idle_timeout
        max_deadline = time.monotonic() + max_timeout
 
        while time.monotonic() < max_deadline:
            if shell.recv_ready():
                drained += shell.recv(65535).decode(errors="ignore")
                idle_deadline = time.monotonic() + idle_timeout
        ...
        if drained:
            print(f"[TESIRA DRAIN] {self._clean_output(drained)[:120]!r}")
    ...
    def _recv_until_done(self, shell, first_byte_timeout=_FIRST_BYTE_TIMEOUT,
        ...
        while True:
            if shell.recv_ready():
                ...
                output   += chunk
                got_first = True
                if self._has_terminator(output):
                    got_terminator = True
                    idle_deadline = time.monotonic() + inter_chunk_timeout
            else:
                now = time.monotonic()
                if got_terminator and idle_deadline and now >= idle_deadline:
                    break
                if not got_first and now >= first_deadline:
                    break
                if got_first and not got_terminator and now >= first_deadline:
                    break
                time.sleep(0.02)
    ...
    def _send_command(self, shell, command):
        ...
        shell.sendall((command + "\r\n").encode())
        raw     = self._recv_until_done(shell)
        cleaned = self._clean_output(raw.replace(command, ""))
        print(f"[TESIRA] {command!r} -> {cleaned[:120]!r}")
        return cleaned
    ...
    def get_device_info(self, ip, port=22, display_id=None):
        ...
        try:
            ...
            result = {
                ...
                "ssh_enabled":            nf("sshDisabled")    == "false",
                "mdns_enabled":           nf("mDNSEnabled")    == "true",
                "fault_status":           fault_name or "No Faults",
                "reachable":              True,
                "raw_network":            net_resp,
                "raw_faults":             flt_resp,
                "raw_audio_blocks":       alias_resp,
