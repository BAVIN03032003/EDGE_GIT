...
#     def _ping_host(self, ip, port=443):
#         """Helper method to check host reachability via TCP socket (works in cloud environments)"""
#         try:
...
#             "design_status": None,
#             "uptime": None,
#             "current_status": "Online" if is_online else "Unknown",
#         }
       
#         try:
#             # Always attempt API calls - device may be reachable even if ping/socket check failed
...
#         try:
#             # Always attempt API call - don't rely on ping
#             root = self._get_status_xml(ip, port)
...
class QSYSCorePlugin(ManualPlatformPlugin):
    ...
    def _get_config_value(self, *keys):
        """Read config values from common direct/nested/env styles."""
        sources = [self.config] if isinstance(self.config, dict) else []
        for nested_key in ("plugin_config", "credentials", "auth"):
            nested = self.config.get(nested_key) if isinstance(self.config, dict) else None
    ...
    def _get_credentials(self):
        username = self._get_config_value("username", "user", "http_username")
        password = self._get_config_value("password", "pass", "http_password")
        return username, password
 
    def _timeout(self):
        try:
            return int(self.config.get("timeout", self.timeout))
        except Exception:
            return self.timeout
 
    def _base_urls(self, ip, port):
        if int(port) == 80:
            schemes = ("http", "https")
        elif int(port) == 443:
            schemes = ("https", "http")
            ...
            schemes = ("https", "http")
        return [f"{scheme}://{ip}:{port}" for scheme in schemes]
 
    def _auth_options(self, username=None, password=None):
        options = [None]
        if username and password:
            options.extend([
    ...
    def _get_status_xml(self, ip, port, username=None, password=None):
        """Get the status XML from the device"""
        errors = []
        for base_url in self._base_urls(ip, port):
            url = f"{base_url}/cgi-bin/status_xml"
            for auth in self._auth_options(username, password):
                try:
        ...
        self.last_error = "; ".join(errors[-4:]) if errors else "status_xml unavailable"
        logger.debug("Q-SYS status_xml failed for %s:%s: %s", ip, port, self.last_error)
        return None
        ...
        for base_url in self._base_urls(ip, port):
            try:
                ...
                if response.status_code in [200, 201]:
                    ...
                    self.token_expiry = time.time() + 3600
                    return self.token
                errors.append(f"{base_url}/api/v0/logon returned HTTP {response.status_code}")
            except Exception as exc:
                errors.append(f"{base_url}/api/v0/logon failed: {exc}")
 
        self.last_error = "; ".join(errors[-4:]) if errors else "logon unavailable"
        return None
        ...
        if token:
            for base_url in self._base_urls(ip, port):
                try:
                    headers = {"Authorization": f"Bearer {token}"}
                    url = f"{base_url}/api/v0/cores"
                    response = self.session.get(url, timeout=self._timeout(), verify=False, headers=headers)
                    ...
                    if response.status_code == 200:
                        ...
                        self.last_error = f"{url} returned HTTP {response.status_code}"
                except Exception as exc:
                    self.last_error = f"{base_url}/api/v0/cores failed: {exc}"
       
        # Method 2: Direct Digest Auth on /api/v0/cores/self (fallback)
        for base_url in self._base_urls(ip, port):
            url = f"{base_url}/api/v0/cores/self"
            for auth in self._auth_options(username, password)[1:]:
                try:
                    response = self.session.get(url, timeout=self._timeout(), verify=False, auth=auth)
                    if response.status_code == 200:
                        data = response.json()
                        serial = data.get("serialNo")
                        if serial and len(serial) > 5 and not self._is_mac_address(serial):
                    ...
                    self.last_error = f"{url} failed: {exc}"
    ...
    def _get_peripheral_serial(self, ip, port, username, password):
        """Get serial number from peripheral device using env_show endpoint"""
        for base_url in self._base_urls(ip, port):
            url = f"{base_url}/api/v1/env_show?serialno"
            for auth in self._auth_options(username, password):
                try:
                    response = self.session.get(
                        url,
    ...
    def get_device_info(self, ip, port=443, display_id=None):
        ...
        try:
            # Always attempt API calls - device may be reachable even if ping/socket check failed
            root = self._get_status_xml(ip, port, username, password)
        ...
        if self.last_error and not root:
            device_info["error"] = self.last_error
    ...
    def query_status(self, ip, port=443, display_id=None):
        ...
        try:
            ...
            if root:
                ...
                status["reachable"] = True
                status["error"] = self.last_error or "API requires authentication or is not accessible"
            else:
                status["error"] = "Device is offline or unreachable"
